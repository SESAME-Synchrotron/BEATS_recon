...
from recon_utils import recon_utils as ru
...
def read_phase_retrieval_params(h5file):
    try:
        pixel_size = dxchange.read_hdf5(
            h5file, "/measurement/instrument/camera/pixel_size"
        )[0]
    ...
    try:
        magnification = dxchange.read_hdf5(
            h5file, "/measurement/instrument/detection_system/objective/magnification"
        )[0]
    except:
        logging.error("Cannot read detector magnification from HDF5 file.")
        magnification = 0

    dist = dxchange.read_hdf5(
        h5file, "/measurement/instrument/detector_motor_stack/detector_z"
    )[0]
...
def main():
    ...
    parser.add_argument(
        "h5file", type=str, help="<Required> Input HDF5 filename (projection data)."
    )
    parser.add_argument(
        "-hts",
        "--h5file_to_subtract",
    ...
    parser.add_argument(
        "--recon_dir", type=str, default=None, help="Output reconstruction folder."
    )
    parser.add_argument(
        "--average", type=int, default=None, help="Average each n projections."
    )
    parser.add_argument(
        "--phase",
        dest="phase",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.001,
        help="Phase retrieval regularization parameter.",
    )
    parser.add_argument(
        "--pixelsize",
        type=float,
    ...
    parser.add_argument(
        "--360", dest="fullturn", action="store_true", help="360 degrees scan."
    )
    parser.add_argument(
        "--overlap", type=int, default=0, help="Overlap parameter for 360 degrees scan."
    )
    parser.add_argument(
        "--rotside",
        type=str,
        default="left",
        help="Rotation axis side for 360 degrees scan.",
    )
    parser.add_argument(
        "--simulate_theta",
        dest="simuate_theta",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Add offset to theta array (degrees).",
    )
    ...
    parser.add_argument(
        "--nchunk", type=int, default=None, help="Chunk size for each core."
    )
    parser.add_argument(
        "--algorithm",
        type=str,
    ...
    parser.add_argument(
        "--size", type=int, default=51, help="Window size of the median filter."
    )
    parser.add_argument(
        "--drop_ratio",
        type=float,
    ...
    parser.add_argument(
        "--circ_mask_val", type=float, default=0.0, help="Value for the masked region."
    )
    parser.add_argument(
        "--output",
        dest="output",
    ...
    parser.set_defaults(
        fullturn=False,
        simulate_theta=False,
        phase=False,
        phase_pad=True,
        circ_mask=False,
        write_midplanes=False,
        verbose=False,
        norm=True,
        output=True,
        flats_scale=False,
    ...
    if not os.path.isdir(work_dir):
        logging.warning(
            "Work directory does not exist. Will create it: {}".format(work_dir)
        )
    ...
    if not os.path.isdir(recon_dir):
        logging.warning(
            "Reconstruction directory does not exist. Will create it: {}".format(
                recon_dir
            )
        )
    ...
    projs, flats, darks, _ = dxchange.read_aps_32id(
        args.h5file, exchange_rank=0, proj=args.proj, sino=args.sino
    )
    theta = np.radians(
        dxchange.read_hdf5(args.h5file, "exchange/theta", slc=(args.proj,))
    )
    ...
    if args.flats_separate:
        _, flats, _, _ = dxchange.read_aps_32id(
            args.flats_separate, exchange_rank=0, proj=args.proj, sino=args.sino
        )
        logging.info(
            "Flat field loaded from separate file: {0}\n".format(args.flats_separate)
        )

        # scale the flat field
        if args.flats_scale:
            current_scan = dxchange.read_hdf5(
                args.h5file, "/measurement/instrument/source/current"
            )[0]
            current_flats = dxchange.read_hdf5(
                args.flats_separate, "/measurement/instrument/source/current"
            )[0]
            ...
            logging.info(
                "Flat field scaled with factor: {0}\n".format(
                    (current_scan / current_flats)
                )
            )

    # separate dark field file
    if args.darks_separate:
        _, _, darks, _ = dxchange.read_aps_32id(
            args.darks_separate, exchange_rank=0, proj=args.proj, sino=args.sino
        )
        logging.info(
            "Dark field loaded from separate file: {0}\n".format(args.darks_separate)
        )
    ...
    if args.stripe_method is not None:
        if "dead" in args.stripe_method:
            ...
            projs = tomopy.prep.stripe.remove_dead_stripe(
                projs,
                snr=args.snr,
                size=args.size,
                ncore=args.ncore,
            ...
            logging.info(
                "Remove large stripe artifacts from sinogram using Nghia Vo’s approach...\n"
            )
            projs = tomopy.prep.stripe.remove_large_stripe(
                projs,
                snr=args.snr,
                size=args.size,
            ...
            projs = tomopy.prep.stripe.remove_stripe_based_sorting(
                projs,
                size=args.size,
                dim=args.dim,
                ncore=args.ncore,
            ...
            logging.info(
                "Remove all types of stripe artifacts from sinogram using Nghia Vo’s approach...\n"
            )
            projs = tomopy.prep.stripe.remove_all_stripe(
                projs,
                snr=args.snr,
    ...
    if args.fullturn:
        logging.info(
            "Convert 360 to 180 degrees sinograms. Rotation axis: {0}; Overlap: {1}.\n".format(
                args.rotside, args.overlap
            )
        )
    ...
    if args.theta_offset is not None:
        ...
        logging.info(
            "Added {0} degree offset to theta array.\n".format(args.theta_offset)
        )
    ...
    if args.phase:
        ...
        logging.info(
            "Attempting to read phase retrieval parameters from HDF5 file...\n"
        )
        det_pixel_size, magnification, dist, energy = read_phase_retrieval_params(
            args.h5file
        )
        ...
        logging.info(
            "	- Detector pixel size: {0} micron".format(det_pixel_size * 1e3)
        )
        ...
        # Retrieve phase
        projs = tomopy.retrieve_phase(
            projs,
            pixel_size=0.1 * pixel_size,
            dist=0.1 * dist,
            energy=energy,
            alpha=args.alpha,
            pad=args.phase_pad,
    ...
    if args.h5file_to_subtract:
        ...
        projs_to_sub, flats_to_sub, darks_to_sub, _ = dxchange.read_aps_32id(
            args.h5file_to_subtract, exchange_rank=0, proj=args.proj, sino=args.sino
        )
        projs_to_sub = tomopy.normalize(
            projs_to_sub, flats_to_sub, darks_to_sub, ncore=args.ncore
        )
    ...
    logging.info(
        "Sinogram size: {2} x {1} x {0} [X x Y x Theta]".format(
            projs_shape[0], projs_shape[1], projs_shape[2]
        )
    )

    preprocess_time = recon_start_time - time_start
    logging.info(
        "Dataset loaded and preprocessed in {} s\n".format(str(preprocess_time))
    )
    logging.info("Start reconstruction with algorithm: " + args.algorithm)
    if "cuda_astra" in args.algorithm:
        if "fbp" in args.algorithm:
            ...
            logging.warning(
                "Algorithm option not recognized. Will reconstruct with ASTRA FBP CUDA."
            )
            options = {"proj_type": "cuda", "method": "FBP_CUDA"}
        recon = tomopy.recon(
            projs,
    ...
    logging.info(
        "Reconstructed volume size: {2} x {1} x {0} [X x Y x Z]".format(
            recon_shape[0], recon_shape[1], recon_shape[2]
        )
    )

    if args.circ_mask:
        # apply circular mask
        recon = tomopy.circ_mask(
            recon, axis=0, ratio=args.circ_mask_ratio, val=args.circ_mask_val
        )
        logging.info(
            "Applied circular mask of {}%.\n".format(100 * args.circ_mask_ratio)
        )
    ...
    if "uint" in args.dtype:
        ...
        if args.data_range:
            logging.info(
                "Data range for rescale given by user: {}.\n".format(args.data_range)
            )
        recon = ru.touint(
            recon, args.dtype, args.data_range, args.data_quantiles, nchunk=args.nchunk
        )
    ...
    if args.write_midplanes:
        ru.writemidplanesDxchange(
            recon, os.path.dirname(recon_dir) + ".tiff", dtype=args.dtype
        )
